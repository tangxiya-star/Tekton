 ...
 *
 * Derives ONE artifacts/structural-spec.notre-dame.json from
 * data/notre-dame-canonical.json + the Gothic ruleset. Deterministic, zero API calls.
 *
 ...
 * + artifacts/derivation-stream.notre-dame.jsonl (one event per component, in order).
 */
...
L(``);
L(`Rule Engine COMPOSER run. Inputs: \`data/notre-dame-canonical.json\` (verified corpus, 16 verdicts / 40 gaps) + the Gothic ruleset.`);
...
const spec = {
  meta: {
    ...
    generated_by: "scripts/derive-notre-dame.mjs (Yingzao Rule Engine — whole-cathedral composer; PASS 1 massing + spire)",
...
console.log(`derive (notre-dame, whole cathedral): ${components.length} components → artifacts/structural-spec.notre-dame.json`);
