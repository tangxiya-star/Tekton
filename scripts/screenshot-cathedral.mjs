 ...
 *  the 3D scene. RE-KEYED to ?building=notre-dame-whole (the separate cathedral tab). */
...
for (const cam of ["default", "front", "aerial", "detail"]) {
  await page.goto(`${URL}/?building=notre-dame-whole&cam=${cam}`, { waitUntil: "domcontentloaded" });
  await ready();
...
await page.goto(`${URL}/?building=notre-dame-whole&cam=default`, { waitUntil: "domcontentloaded" });
await ready();
