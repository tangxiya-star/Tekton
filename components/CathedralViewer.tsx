...
/**
 * CathedralViewer — procedural R3F reconstruction of the WHOLE Notre-Dame de Paris
 * (PASS 1 massing + the full-fidelity flèche on the crossing).
 *
 * RE-KEYED (notre-dame-whole): the SEPARATE whole-cathedral viewer. Renders ONLY
 * from artifacts/structural-spec.notre-dame-whole.json + verifier-report.notre-dame-whole.json.
 * The focused spire (components/SpireViewer.tsx → structural-spec.notre-dame.json)
 * is left untouched and stays spire-only. This component is byte-equivalent to the
 * whole-branch SpireViewer (the HAS_MASSING branch frames the whole silhouette);
 * only the spec/report imports point at the -whole artifacts.
 *
 * No imported meshes — every component is procedural geometry. Each mesh carries
 * userData {componentId, provenance, source, citation} for inspection.
 *
...
import spec from "../artifacts/structural-spec.notre-dame-whole.json";
import report from "../artifacts/verifier-report.notre-dame-whole.json";
...
export default function CathedralViewer() {
